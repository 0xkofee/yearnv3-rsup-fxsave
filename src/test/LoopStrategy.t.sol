...
contract LoopStrategyTest is Setup {
    ...
    /// @notice Proves that minLoopAmount threshold can leave dust stuck in the strategy
    function testMinLoopAmount_CanLeaveDustStuck() public {
        // 1. Deposit funds and create leveraged position
        vm.startPrank(user);
        reUSD.approve(address(loopStrategy), INITIAL_DEPOSIT);
        strategyVault.deposit(INITIAL_DEPOSIT, user);
        vm.stopPrank();

        emit log("=== After Initial Deposit ===");
        _logPositions();

        uint256 userShares = strategyVault.balanceOf(user);
        uint256 totalAssetsBefore = strategyVault.totalAssets();
        emit log_named_decimal_uint("User shares", userShares, 18);
        emit log_named_decimal_uint("Total assets before", totalAssetsBefore, 18);

        // 2. Withdraw most funds, leaving just above minLoopAmount
        // minLoopAmount = 1100e18, so leaving ~1000 reUSD worth should cause issues
        uint256 withdrawAmount = totalAssetsBefore - 500e18; // Leave only 500 reUSD worth
        uint256 sharesToRedeem = strategyVault.convertToShares(withdrawAmount);

        emit log("=== Attempting Large Withdrawal ===");
        emit log_named_decimal_uint("Attempting to withdraw", withdrawAmount, 18);
        emit log_named_decimal_uint("Shares to redeem", sharesToRedeem, 18);

        vm.prank(user);
        uint256 assetsReceived = strategyVault.redeem(sharesToRedeem, user, user);
        emit log_named_decimal_uint("Assets received", assetsReceived, 18);

        emit log("=== After Large Withdrawal ===");
        _logPositions();

        uint256 remainingShares = strategyVault.balanceOf(user);
        uint256 totalAssetsAfter = strategyVault.totalAssets();
        emit log_named_decimal_uint("Remaining user shares", remainingShares, 18);
        emit log_named_decimal_uint("Total assets after", totalAssetsAfter, 18);

        // 3. Now try to withdraw the remaining small amount
        emit log("=== Attempting Final Withdrawal (Dust) ===");

        if (remainingShares > 0) {
            vm.prank(user);
            uint256 finalAssets = strategyVault.redeem(remainingShares, user, user);
            emit log_named_decimal_uint("Final assets received", finalAssets, 18);

            // Check if there's still value locked in strategy
            uint256 totalAssetsRemaining = strategyVault.totalAssets();
            emit log_named_decimal_uint("Assets remaining in strategy", totalAssetsRemaining, 18);

            // Check for dust in various places
            uint256 idleReUSD = reUSD.balanceOf(address(loopStrategy));
            uint256 idleCrvUSD = crvUSD.balanceOf(address(loopStrategy));
            uint256 idleSreUSD = sreUSD.balanceOf(address(loopStrategy));

            emit log("=== Dust Check ===");
            emit log_named_decimal_uint("Idle reUSD", idleReUSD, 18);
            emit log_named_decimal_uint("Idle crvUSD", idleCrvUSD, 18);
            emit log_named_decimal_uint("Idle sreUSD", idleSreUSD, 18);

            // Check if Curve position still has dust
            if (curveLLAMMA.loan_exists(address(loopStrategy))) {
                uint256[4] memory curveState = curveLLAMMA.user_state(address(loopStrategy));
                emit log_named_decimal_uint("Curve collateral dust", curveState[0], 18);
                emit log_named_decimal_uint("Curve debt dust", curveState[1], 18);

                // THIS IS THE BUG: position can't be unwound because amounts are below minLoopAmount
                if (curveState[0] > 0 || curveState[1] > 0) {
                    emit log("WARNING: Curve position has dust that cannot be withdrawn!");
                }
            }
            ...
            uint256 resupplyShares = resupplyPair.userBorrowShares(address(loopStrategy));
            if (resupplyCollateral > 0 || resupplyShares > 0) {
                emit log_named_decimal_uint("Resupply collateral dust", resupplyCollateral, 18);
                emit log_named_decimal_uint("Resupply borrow dust", resupplyShares, 18);
                emit log("WARNING: Resupply position has dust that cannot be withdrawn!");
            }

            // The test passes if we identify dust is stuck
            // A proper implementation should ensure totalAssetsRemaining == 0 after full withdrawal
            uint256 totalDust = idleReUSD + idleCrvUSD + idleSreUSD;
            if (curveLLAMMA.loan_exists(address(loopStrategy))) {
                uint256[4] memory state = curveLLAMMA.user_state(address(loopStrategy));
                totalDust += state[0]; // sreUSD collateral
            }
            totalDust += resupplyCollateral;

            emit log_named_decimal_uint("TOTAL DUST STUCK", totalDust, 18);

            // This assertion documents the bug - dust CAN get stuck
            // When fixed, this should be: assertEq(totalDust, 0, "No dust should remain");
            if (totalDust > 0) {
                emit log("BUG CONFIRMED: minLoopAmount threshold leaves dust stuck!");
            }
        }
    }
